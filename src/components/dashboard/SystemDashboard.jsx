...
/**
 * en-US, never the ambient locale.
 *
 * `toLocaleString(undefined, ...)` renders Bengali digits on a machine set to Bengali, so
 * the same figure read differently here than on every other screen, and the PDF export
 * inherited it. The taka sign is deliberate and stays: it lives in the Bengali block, so
 * the PDF surface routes it to the embedded Bengali face, the only one carrying the glyph.
 */
const money = (n) =>
...
const SystemDashboard = () => {
  ...
  const { user } = useAuth();
  // The PDF export is asynchronous - it may fetch a Bengali typeface - so the button reports
  // its own progress rather than appearing inert.
  ...
  const handleExportPDF = async () => {
    ...
    try {
      setExportError(null);
      setExporting(true);
      const { generateSystemReportPdf, systemReportFileName } = await import('../../utils/systemReportPdf');
      const surface = await generateSystemReportPdf({ data, generatedBy: user?.name, money });
      await surface.save(systemReportFileName());
    } catch (err) {
      // The export fetches a Bengali typeface when the report contains any, so it can fail
      // for reasons unrelated to the data. Saying so beats a button that does nothing.
      console.error('System report export failed', err);
      setExportError(t('export_failed'));
    } finally {
      setExporting(false);
    }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-4 justify-between items-start">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Btn type="secondary" onClick={handleExportPDF} disabled={exporting}>
            {exporting ? t('generating') : t('export_report')}
          </Btn>
          <Btn type="primary" href="/admin/generate-token">{t('generate_invitation_link')}</Btn>
        </div>
        {exportError && (
          <p className="w-full rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
            {exportError}
          </p>
        )}
      </div>
